...
namespace Kanng.Common
{
    ...
    public class BrowserHelper
    {
        ...
        /// </summary>
        /// <param name="url"></param>
        public static void Open360(string url)
        {
            try
            {
                // 64位注册表路径
                var openKey = @"SOFTWARE\Classes\WOW6432Node\CLSID\{0002DF01-0000-0000-C000-000000000046}\LocalServer32";
                if (IntPtr.Size == 4)
                {
                    // 32位注册表路径
                    openKey = @"SOFTWARE\Classes\CLSID\{0002DF01-0000-0000-C000-000000000046}\LocalServer32";
                }
                RegistryKey appPath = Registry.LocalMachine.OpenSubKey(openKey);
                var value = appPath.GetValue("").ToString();
                if (appPath != null)
                {
                    // IE浏览器路径安装：C:\Program Files\Internet Explorer
                    // at System.Diagnostics.process.StartWithshellExecuteEx(ProcessStartInfo startInfo)注意这个错误
                    ...
                    {
                        if (MessageBox.Show(@"系统未安装360浏览器，是否下载安装？", null, MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question) == DialogResult.Yes)
                        {
                            // 打开下载链接，从微软官网下载
                            OpenDefaultBrowserUrl("http://se.360.cn/");
                        }
                    }
                }
                else
